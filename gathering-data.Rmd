...
mlb_scores_1947 <- read_csv("data/mlbscores/mlb_elo.csv", 
  ...
  mutate(away_team = case_when(away_team_id == "HOU" ~ "Houston Astros",
                               ...
                               away_team_id == "LAA" ~ "Lost Angeles Angels",
...
```{r mlb interactive model test}


        # Creating MLB interactive model data.

mlb_model_int <- mlb_model_complex %>%
  as_tibble() %>%
  rename("mu" = "(Intercept)",
  ...
  mutate(mu_median = median(mu)) %>%
  mutate(predicted_home_median = median(predicted_home)) %>%
  mutate(predicted_attendance_median = median(predicted_attendance)) %>%
  mutate(predicted_interaction_median = median(predicted_interaction)) %>%
